...
Linear models assume: \texttt{output = w1*feature1 + w2*feature2 + ...}
...
# =============================================================================
# Step 2: Define a 2-layer neural network
# =============================================================================
...
def train_model(model, optimizer, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        print(f"  Epoch {epoch+1}/{epochs} -- "
              f"Loss: {avg_loss:.4f}, Train Acc: {accuracy:.4f}")
...
# =============================================================================
# Experiment 1: Good defaults (should work)
# =============================================================================
print("\n" + "=" * 60)
print("EXPERIMENT 1: Good defaults (ReLU + Adam + lr=0.001)")
print("=" * 60)

model_good = SimpleNet(hidden_size=128, activation='relu')
optimizer_good = optim.Adam(model_good.parameters(), lr=0.001)
train_model(model_good, optimizer_good, epochs=5)
acc_good = evaluate(model_good)
print(f"  Test Accuracy: {acc_good:.4f}")

# =============================================================================
# Experiment 2: Bad activation (sigmoid -- vanishing gradients)
# =============================================================================
print("\n" + "=" * 60)
print("EXPERIMENT 2: Sigmoid activation (watch it struggle)")
print("=" * 60)

model_sigmoid = SimpleNet(hidden_size=128, activation='sigmoid')
optimizer_sigmoid = optim.Adam(model_sigmoid.parameters(), lr=0.001)
train_model(model_sigmoid, optimizer_sigmoid, epochs=5)
acc_sigmoid = evaluate(model_sigmoid)
print(f"  Test Accuracy: {acc_sigmoid:.4f}")

# =============================================================================
# Experiment 3: Learning rate too high (training explodes)
# =============================================================================
print("\n" + "=" * 60)
print("EXPERIMENT 3: Learning rate too high (lr=1.0)")
print("=" * 60)

model_highLR = SimpleNet(hidden_size=128, activation='relu')
optimizer_highLR = optim.SGD(model_highLR.parameters(), lr=1.0)
train_model(model_highLR, optimizer_highLR, epochs=5)
acc_highLR = evaluate(model_highLR)
print(f"  Test Accuracy: {acc_highLR:.4f}")
...
print("SUMMARY")
print("=" * 60)
print(f"{'Setup':<40} {'Test Accuracy':<15}")
print("-" * 55)
print(f"{'ReLU + Adam + lr=0.001':<40} {acc_good:<15.4f}")
print(f"{'Sigmoid + Adam + lr=0.001':<40} {acc_sigmoid:<15.4f}")
print(f"{'ReLU + SGD + lr=1.0':<40} {acc_highLR:<15.4f}")
print("-" * 55)
\end{lstlisting}
...
============================================================
EXPERIMENT 1: Good defaults (ReLU + Adam + lr=0.001)
============================================================
  Epoch 1/5 -- Loss: 0.3412, Train Acc: 0.9032
  Epoch 2/5 -- Loss: 0.1534, Train Acc: 0.9547
  Epoch 3/5 -- Loss: 0.1098, Train Acc: 0.9672
  Epoch 4/5 -- Loss: 0.0856, Train Acc: 0.9742
  Epoch 5/5 -- Loss: 0.0701, Train Acc: 0.9789
  Test Accuracy: 0.9734

============================================================
EXPERIMENT 2: Sigmoid activation (watch it struggle)
============================================================
  Epoch 1/5 -- Loss: 0.5623, Train Acc: 0.8345
  Epoch 2/5 -- Loss: 0.2876, Train Acc: 0.9178
  Epoch 3/5 -- Loss: 0.2234, Train Acc: 0.9356
  Epoch 4/5 -- Loss: 0.1867, Train Acc: 0.9452
  Epoch 5/5 -- Loss: 0.1623, Train Acc: 0.9523
  Test Accuracy: 0.9501

============================================================
EXPERIMENT 3: Learning rate too high (lr=1.0)
============================================================
  Epoch 1/5 -- Loss: 1.8234, Train Acc: 0.4523
  Epoch 2/5 -- Loss: 0.8912, Train Acc: 0.7234
  Epoch 3/5 -- Loss: 0.6534, Train Acc: 0.7892
  Epoch 4/5 -- Loss: 0.5678, Train Acc: 0.8123
  Epoch 5/5 -- Loss: 0.4923, Train Acc: 0.8345
...
Setup                                    Test Accuracy
-------------------------------------------------------
ReLU + Adam + lr=0.001                   0.9734
Sigmoid + Adam + lr=0.001                0.9501
ReLU + SGD + lr=1.0                      0.8312
-------------------------------------------------------
\end{lstlisting}

\textbf{What This Demonstrates}:
\begin{itemize}
    \item \textbf{Experiment 1}: Good defaults work. ReLU avoids vanishing gradients, Adam adapts the learning rate, and $\text{lr}=0.001$ is a safe starting point. You get $\sim$97\% test accuracy on MNIST with a simple 2-layer network.
    \item \textbf{Experiment 2}: Sigmoid hurts. The gradients saturate for large activations, slowing learning. It still converges (Adam is forgiving), but it's noticeably worse.
    \item \textbf{Experiment 3}: High learning rate with SGD is chaotic. The model overshoots the loss landscape. It still learns \textit{something}, but performance is much worse.
\end{itemize}

\begin{insightbox}
\textbf{Key Insight}: Neural networks are finicky. Small details (initialization, learning rate, activation) make the difference between working and not working. Always start with proven defaults: ReLU activation, Adam optimizer, learning rate $\sim$0.001.
\end{insightbox}
