...
\section{Decision Trees: Thinking in Questions}

Linear models draw straight lines through data. But what if the real boundary is ``if income $>$ 50k AND debt ratio $<$ 0.3''? That's not a line---it's a set of conditions. Decision trees think in questions.

\subsection{How a Decision Tree Works}

A decision tree splits data by asking yes/no questions, one feature at a time:

\begin{enumerate}
    \item Start with all data at the root.
    \item Find the feature and threshold that best separates the classes.
    \item Split into two branches.
    \item Repeat recursively on each branch until some stopping criterion.
\end{enumerate}

\textbf{What does ``best separates'' mean?} Two common criteria:

\begin{itemize}
    \item \textbf{Gini impurity}: Measures how often a randomly chosen element would be misclassified. For a node with class proportions $p_1, p_2, \ldots, p_k$:
    $$\text{Gini} = 1 - \sum_{i=1}^{k} p_i^2$$
    A pure node (all one class) has Gini $= 0$. Maximum impurity when classes are evenly split.

    \item \textbf{Information gain (entropy)}: How much ``surprise'' is reduced by the split:
    $$\text{Entropy} = -\sum_{i=1}^{k} p_i \log_2 p_i$$
    The split that reduces entropy the most wins.
\end{itemize}

\begin{asidebox}
\textbf{In practice}: Gini and entropy give nearly identical results. Scikit-learn uses Gini by default. Don't overthink this choice.
\end{asidebox}

\subsection{Decision Trees in Code}
...
# Generate a dataset with informative and noisy features
...
# Pruned tree: max_depth limits complexity
...
print("-" * 50)
print(f"{'Unrestricted tree':<25} "
      f"{accuracy_score(y_train, tree_deep.predict(X_train)):<12.4f} "
      f"{accuracy_score(y_test, tree_deep.predict(X_test)):<12.4f}")
print(f"{'Pruned (depth=5)':<25} "
      f"{accuracy_score(y_train, tree_pruned.predict(X_train)):<12.4f} "
      f"{accuracy_score(y_test, tree_pruned.predict(X_test)):<12.4f}")

# Feature importances: which features does the tree rely on?
importances = tree_pruned.feature_importances_
print("\nTop 5 Feature Importances (pruned tree):")
...
--------------------------------------------------
Unrestricted tree         1.0000       0.8750
Pruned (depth=5)          0.9375       0.8900
...
Notice: the unrestricted tree gets \textit{perfect} training accuracy but worse test accuracy. It memorized the training data. The pruned tree trades a bit of training accuracy for better generalization.

\subsection{The Decision Tree Failure Mode}

Decision trees are \textit{high variance} learners. Small changes in training data can produce wildly different trees. This instability makes single decision trees unreliable for production use.

But this weakness becomes a strength when combined with ensembles.

\section{Ensemble Methods: The Wisdom of Crowds}

One model is fragile. Many models averaged together are robust. This is the core insight of ensemble methods---and it's why gradient boosted trees dominate Kaggle competitions and production ML.

\subsection{Random Forests: Averaging Away Variance}

A random forest is embarrassingly simple:

\begin{enumerate}
    \item Create $N$ bootstrap samples (random subsets with replacement) from the training data.
    \item Train one decision tree on each sample, but at each split only consider a random subset of features.
    \item For prediction, average all trees (regression) or take a majority vote (classification).
\end{enumerate}

\textbf{Why does this work?} Each tree is a high-variance estimator. But because they're trained on different data subsets with different feature subsets, their errors are \textit{uncorrelated}. Averaging uncorrelated errors reduces variance without increasing bias.

Formally, if each tree has variance $\sigma^2$ and the correlation between trees is $\rho$, the variance of the ensemble of $N$ trees is:
$$\text{Var}_{\text{ensemble}} = \rho \sigma^2 + \frac{1 - \rho}{N} \sigma^2$$

As $N$ increases, the second term shrinks. The random feature selection at each split reduces $\rho$. Both effects reduce ensemble variance.
...
\subsection{Gradient Boosting: Learning From Mistakes}

Random forests build trees in parallel and average them. Gradient boosting builds trees \textit{sequentially}, where each new tree corrects the errors of the previous ones.

The algorithm:
\begin{enumerate}
    \item Start with a simple prediction (e.g., the mean).
    \item Compute residuals: where is the current model wrong?
    \item Fit a new (shallow) tree to predict those residuals.
    \item Add this tree's predictions to the running total (scaled by learning rate $\eta$).
    \item Repeat.
\end{enumerate}
...
where $F_0$ is the initial prediction, $h_m$ are the correction trees, and $\eta$ is the learning rate.

\begin{asidebox}
\textbf{Why ``gradient''?} Each tree is fit to the negative gradient of the loss function with respect to the current predictions. For squared error loss, the negative gradient is just the residual. For other losses (log loss, etc.), it's a different quantity---but the principle is the same: move in the direction that reduces loss.
\end{asidebox}

\subsection{XGBoost and LightGBM: Production Gradient Boosting}

In practice, nobody implements gradient boosting from scratch. Two libraries dominate:

\begin{itemize}
    \item \textbf{XGBoost}: The original ``gradient boosting on steroids.'' Adds regularization to the tree-building objective, handles missing values natively, supports parallel computation.
    \item \textbf{LightGBM}: Microsoft's response. Faster than XGBoost on large datasets due to histogram-based splitting and leaf-wise tree growth (vs.\ XGBoost's level-wise growth).
\end{itemize}
...
models = {
    ...
    'Gradient Boosting (200 trees)': gb,
}
...
Model                          Train Acc    Test Acc
------------------------------------------------------------
Decision Tree (pruned)         0.9375       0.8900
Random Forest (100 trees)      0.9988       0.9200
Gradient Boosting (200 trees)  0.9875       0.9350
------------------------------------------------------------
\end{lstlisting}

The pattern is clear: single tree $<$ random forest $<$ gradient boosting. This ranking holds across most tabular datasets.
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Method} & \textbf{Strengths} & \textbf{Use When} \\
\midrule
Decision Tree & Interpretable, fast & You need explainability, small data \\
Random Forest & Robust, few hyperparams & Good default for tabular data \\
Gradient Boosting & Best accuracy on tabular & Competitions, production (tuned) \\
Linear Models & Very fast, interpretable & High-dimensional, sparse features \\
\bottomrule
\end{tabular}
\end{table}

\begin{warningbox}
\textbf{Common mistake}: Reaching for neural networks on tabular data. On structured/tabular datasets, gradient boosted trees consistently match or beat neural networks. Deep learning shines on images, text, and audio---not on your customer churn spreadsheet.
\end{warningbox}
...
print(f"True function: sin(2*pi*x)")
print(f"Noise level: sigma = 0.3")
print()
...
print(f"\n{'Alpha (L)':<12} {'Train MSE':<15} {'Test MSE':<15} {'Effect'}")
print("-" * 55)
